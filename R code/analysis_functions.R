...
stacked_bar <- function(df, 
  ...
  # Create a plot_n to avoid the case that the count number 
  # in the column is too large to display. 
  # plot_n is the column height in image, while n is the 
  # true count number of the papers. 
  temp_df <- df %>% 
    group_by(!!sym(group_var), !!sym(groupvar_stack)) %>% 
    summarise(Count = n()) %>%
    mutate(plot_n = Count + 3)
  
  # Order the columns by count. 
  if (order_by_count) {
    
    p <- ggplot(data = temp_df, 
                aes(x = reorder(!!sym(group_var), -plot_n, sum),
                    y = plot_n, 
                    fill = !!sym(groupvar_stack))) 
